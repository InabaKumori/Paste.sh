...
paste_file() {
    ...
    local url="http://localhost:55554/$filename.html"
    echo "$url"
}
...
# Start a simple web server using nc (netcat)
cd "$UPLOAD_DIRECTORY"
echo "Serving files on http://localhost:55554"
echo "Serving files on http://$LOCAL_IP:55554"
echo "Press Ctrl+C to stop the server"
while true; do
    echo -e "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n$(cat "$html_file")" | nc -l 55554
done
